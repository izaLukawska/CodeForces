...
public class CodeForces_266B {
    ...
    public static String queue(int kids, int seconds, String sex) {
        ...
        } else if (seconds > 50 && seconds < kids) {
            throw new IllegalArgumentException("Seconds count must be bigger than kids number but less than 50");
        } else {
            LinkedList<String> list = Arrays.stream(sex.split("")).collect(Collectors.toCollection(LinkedList::new));

            for(int j = 0; j < seconds; ++j) {
                for(int i = list.size() - 1; i > 0; --i) {
                    if (((String)list.get(i)).equals("G") && ((String)list.get(i - 1)).equals("B")) {
                        list.set(i, "B");
                        list.set(i - 1, "G");
                    }
                }
